...
Public Class Holidays
    ...
    Private Sub cmdCount_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCount.Click
        ...
        Dim HolidayData() As String = File.ReadAllLines("". . . .".txt")
        For i = 0 To UBound(Holiday)

            CountGot = 0
            If Trim(Mid("HolidayData"(i), 1, 50)) = txtHolidayID.Text And Not txtHolidayID.Text = "" Then CountGot = CountGot + 1
            If Trim(Mid("HolidayName"(i), 51, 50)) = txtHolidayName.Text And Not txtHolidayName.Text = "" Then CountGot = CountGot + 1
            If Trim(Mid("HolidayLocation"(i), 101, 50)) = txtLocation.Text And Not txtLocation.Text = "" Then CountGot = CountGot + 1
            If Trim(Mid("HolidayType"(i), 151, 50)) = txtHolidayType.Text And Not txtHolidayType.Text = "" Then CountGot = CountGot + 1 'Counting how many attributes follow the search
            If Trim(Mid("HolidayRating"(i), 201, 50)) = txtRating.Text And Not txtRating.Text = "" Then CountGot = CountGot + 1
            If CountGot > 0 Then HolidayCount = ". . . ." + 1 ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''If attributes match, add to the count.
        Next i
